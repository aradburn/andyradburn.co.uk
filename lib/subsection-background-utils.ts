...
export function progressToYPercent(
    p: number,
    rect: DOMRect,
    vh: number,
): number {
...
export function aboutProgressToYPercent(
    p: number,
    rect: DOMRect,
    vh: number,
): number {
...
export function sectionOpacity(p: number, rect: DOMRect, vh: number): number {
    ...
    if (p < enterFadeEnd)
        return (
            (OPACITY_MAX * (p - enterFadeStart)) /
            (enterFadeEnd - enterFadeStart)
        );
    if (p <= leaveFadeStart) return OPACITY_MAX;
    if (p < leaveFadeEnd)
        return (
            OPACITY_MAX *
            (1 - (p - leaveFadeStart) / (leaveFadeEnd - leaveFadeStart))
        );
    return 0;
}
