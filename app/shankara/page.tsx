...
export async function generateMetadata(): Promise<Metadata> {
    return buildMetadataForSection("shankara");
}

export default async function SpaceCadetsPage() {
    const config = getSectionConfig("shankara");
    ...
    const section3 =
        config.gigs != null ? (
            <div className="subsection-panel">
                <SectionGigsContent section="shankara" config={config} />
            </div>
        ) : null;
    ...
    const section4 =
        config.videos != null ? (
            <div className="subsection-panel">
                <SectionVideosContent section="shankara" config={config} />
            </div>
        ) : null;
    ...
    return (
        <SectionWithBackgrounds
            config={config}
            className="section-shankara max-w-full"
        >
            <ScrollPinSections
                section1={section1}
                section2={section2}
