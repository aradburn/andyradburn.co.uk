...
export async function SectionGigsContent({
            ...
            {gigsWithHtml.length !== 0 ? (
                <div className="flex flex-col gap-8">
                    {gigsWithHtml.map(({ gig, contentHtml }) => {
                        const gig_section_heading =
                            gig.frontMatter.title ?? heading;
                        return (
                            <article
                ...
                >
                    ...
                    <div className="prose prose-invert max-w-none break-words overflow-x-hidden prose-a:text-accent">
                        <p className="text-text-muted">
                            No upcoming gigs listed.
                        </p>
                        <p className="text-text-muted">
                            Check back later or follow us for announcements.
                        </p>
                    </div>
                </article>
            )}
    ...
    return <div className="max-w-full">{contentBlock}</div>;
}
