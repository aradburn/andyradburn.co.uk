...
vi.mock("gsap", () => ({
    default: {
        to: vi.fn(
            (
                _: unknown,
                opts: { scrollTo?: { y: number }; duration?: number },
            ) => {
                mockScrollTo(opts);
            },
        ),
...
describe("SubsectionNavLinks", () => {
    ...
    it("renders nav with aria-label", () => {
        render(<SubsectionNavLinks links={[{ id: "posts", label: "News" }]} />);
        expect(
    ...
    it("renders links as buttons", () => {
        ...
        expect(
            screen.getByRole("button", { name: "Gigs" }),
        ).toBeInTheDocument();
    });
    ...
    it("does not call gsap when target section does not exist", () => {
        ...
        render(
            <SubsectionNavLinks
                links={[{ id: "nonexistent", label: "Missing" }]}
            />,
        );
